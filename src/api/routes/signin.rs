...
/// Checks and returns if the user is using a username or email to log in  
fn get_auth_cred_method(
    ...
    if username.is_none() && email_id.is_none() {
        return Err(AppError::BadRequest(
            "You need to provide either the 'username' or 'email_id'".into(),
        ));
    }
...
pub async fn login(
    ...
    match check_password {
        ...
        false => Err(AppError::BadRequest("Invalid username or password".into())),
    }
}
